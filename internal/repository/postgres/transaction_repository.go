...
func (r *TransactionRepository) GetByExternalID(ctx context.Context, storeID int64, txType domain.TransactionType, externalTxID string) (*domain.Transaction, error) {
	const query = `
		SELECT id, store_id, client_id, external_tx_id, amount, type, points_delta, balance_after, created_at
		FROM transactions
		WHERE store_id = $1 AND type = $2 AND external_tx_id = $3`

	tx := &domain.Transaction{}
	err := r.pool.QueryRow(ctx, query, storeID, txType, externalTxID).Scan(
		&tx.ID, &tx.StoreID, &tx.ClientID, &tx.ExternalTxID, &tx.Amount, &tx.Type, &tx.PointsDelta, &tx.BalanceAfter, &tx.CreatedAt,
	)
...
func (r *TransactionRepository) ListByClient(ctx context.Context, clientID int64) ([]*domain.Transaction, error) {
	...
	for rows.Next() {
		tx := &domain.Transaction{}
		if err := rows.Scan(&tx.ID, &tx.StoreID, &tx.ClientID, &tx.ExternalTxID, &tx.Amount, &tx.Type, &tx.PointsDelta, &tx.BalanceAfter, &tx.CreatedAt); err != nil {
